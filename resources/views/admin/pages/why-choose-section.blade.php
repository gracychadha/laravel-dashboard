...
@section("content")
                ...
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-striped bg-theme" id="testimonialTable">
                            ...
                            <tbody>
                                @forelse($whyChooseCards as $card)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>
                                            @if($card->image)
                                                <img src="{{ asset('storage/' . $card->image) }}" width="50"
                                        ...
                                        <td class="text-center">
                                            ...
                                            <form action="{{ route('whychoose-section.destroy', $card) }}" method="POST"
                                                class="d-inline">
                                                @csrf @method('DELETE')
    ...
    <div class="modal fade" id="addModal">
        <div class="modal-dialog custom-modal bg-theme-light">
            <form action="{{ route('whychoose-section.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
    ...
    @foreach($whyChooseCards as $card)
                ...
                <div class="modal-content">
                    ...
                    <table class="table table-bordered table-striped mb-0">
                        ...
                        <tr>
                            ...
                            <td>
                                @if($card->image)
                                    <img src="{{ asset('storage/' . $card->image) }}" width="120"
                                        class=" mb-2">
                                @else
        ...
        <div class="modal fade" id="edit{{ $card->id }}">
            <div class="modal-dialog custom-modal">
                <form action="{{ route('whychoose-section.update', $card) }}" method="POST" enctype="multipart/form-data">
                    @csrf @method('PUT')
                    <div class="modal-content">
                        <div class="modal-header bg-theme-light">
                            <h5>Edit section</h5>
                        ...
                        <div class="modal-body">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label>Title</label>
                                    <input type="text" name="name" value="{{ $card->title }}" class="form-control"
                                        required>
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    @if($card->image)
                                        <img src="{{ asset('storage/' . $card->image) }}" width="80"
