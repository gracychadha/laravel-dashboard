...
class AlliedHealthSupportController extends Controller
{
    //
     public function index()
    {
        $AlliedHealthSupports = AlliedHealthSupport::firstOrCreate(
            [],
            [

                'main_title' => 'Allied Health Support',
        ...
        return view('admin.pages.allied-health-support', compact('AlliedHealthSupports'));
    }
    public function update(Request $request)
    {
        $AlliedHealthSupports = AlliedHealthSupport::firstOrFail();
        ...
        if ($request->hasFile('image')) {
            if ($AlliedHealthSupports->image && Storage::disk('public')->exists($AlliedHealthSupports->image)) {
                Storage::disk('public')->delete($AlliedHealthSupports->image);
            }
            $data['image'] = $request->file('image')->store('alliedSupport', 'public');
        }
        $AlliedHealthSupports->update($data);
