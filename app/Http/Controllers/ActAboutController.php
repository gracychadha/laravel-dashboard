...
class ActAboutController extends Controller
{
    //
    //
    public function index()
    {
        $ActAbouts = ActAbout::firstOrCreate(
            [],
            [
        ...
        return view('admin.pages.admin-act-about', compact('ActAbouts'));
    }
    public function update(Request $request)
    {
        $ActAbouts = ActAbout::firstOrFail();
        ...
        if ($request->hasFile('image')) {
            if ($ActAbouts->image && Storage::disk('public')->exists($ActAbouts->image)) {
                Storage::disk('public')->delete($ActAbouts->image);
            }
            $data['image'] = $request->file('image')->store('admin-act', 'public');
        }
        $ActAbouts->update($data);
        return back()->with('success', 'Act About Section Updated Successfully');
    }
}
