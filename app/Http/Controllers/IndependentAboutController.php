...
class IndependentAboutController extends Controller
{
    //
        public function index()
    {
        $IndependentAbouts = IndependentAbout::firstOrCreate(
            [],
            [

                'main_title' => 'SIL About',
        ...
        return view('admin.pages.independent-about', compact('IndependentAbouts'));
    }
    public function update(Request $request)
    {
        $IndependentAbouts = IndependentAbout::firstOrFail();
        ...
        if ($request->hasFile('image')) {
            if ($IndependentAbouts->image && Storage::disk('public')->exists($IndependentAbouts->image)) {
                Storage::disk('public')->delete($IndependentAbouts->image);
            }
            $data['image'] = $request->file('image')->store('care', 'public');
        }
        $IndependentAbouts->update($data);
