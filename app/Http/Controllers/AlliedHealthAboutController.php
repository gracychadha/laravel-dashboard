...
class AlliedHealthAboutController extends Controller
{
    //
    public function index()
    {
        $AlliedHealthAbouts = AlliedHealthAbout::firstOrCreate(
            [],
            [
        ...
        return view('admin.pages.allied-helth-about', compact('AlliedHealthAbouts'));
    }
    public function update(Request $request)
    {
        $AlliedHealthAbouts = AlliedHealthAbout::firstOrFail();
        ...
        if ($request->hasFile('image')) {
            if ($AlliedHealthAbouts->image && Storage::disk('public')->exists($AlliedHealthAbouts->image)) {
                Storage::disk('public')->delete($AlliedHealthAbouts->image);
            }
            $data['image'] = $request->file('image')->store('allied', 'public');
        }
        $AlliedHealthAbouts->update($data);
